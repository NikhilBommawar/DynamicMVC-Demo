    ...
    this.setWidgets = function () {
       ...
       */
        ...
        for (let i = 0; i < fnBtns.length; i++) {
            var fbtn = new Button('fnBtn_' + this.id + '_' + fnBtns[i],
             { title: fnBtns[i], value: fnBtns[i] ,className: "btn btn-danger function" });
            var _this = this;
       ...
       */
        ...
        for (let i = 0; i < numBtns.length; i++) {
            var numBtn = new Button('numBtn_'+[i] +'_'+ this.id , { title: numBtns[i], value: numBtns[i], className: 'btn btn-primary numerical' });
            var _this = this;
            numBtn.onBtnPress = function (key, value) {
      ...
      */
        ...
        for (let i = 0; i < operationBtns.length; i++) {
            var operationBtn = new Button('opBtn_' + this.id + '_' + operationBtns[i], { title: operationBtns[i], value: operationBtns[i], className: 'btn btn-warning operations' });
            var _this = this;
            operationBtn.onBtnPress = function (key, value) {
